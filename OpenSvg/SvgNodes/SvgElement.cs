...
public abstract class SvgElement : IXmlSerializable
{
    ...
    public StringAttr ID { get; set; } = new(SvgNames.ID, "", isConstant: false);

    public SvgDocument RootDocument => Root as SvgDocument ?? throw new InvalidOperationException("Element does not have a SvgDocument as root element");
    ...
    /// <remarks>
    /// <list type="bullet">
    /// <description>If the element is an <see cref="SvgDocument"/> , the viewport will be defined by its explicit size: 
    ///     <list type="bullet">
    ///     <item>If the <see cref="SvgDocument"/>'s value is absolute, that value is returned</item>
    ///     <item>If the <see cref="SvgDocument"/>'s value is a ratio (a percentage value), query ancestors until an absolute value can be resolved</item>    
    ///     </list>
    /// </description>
    /// <item>If the element is not an SvgDocument, the viewport will be equal to its closest SvgDocument ancestor.</item>
    /// <item>If the closest SvgDocument ancestor has a relative defined size (e.g., <c>40%</c>), the viewport will be calculated by traversing upwards, accumulating ratios until an ancestor with an absolute size is found.</item>
    ...
    /// </summary>
    /// <returns>An instance of <see cref="SvgGroup"/> if the current object is an <see cref="SvgGroup"/> or a new <see cref="SvgGroup"/> object created from the current object.</returns>
    ...
    public virtual (bool Equal, string Message) CompareSelfAndDescendants(SvgElement other, double doublePrecision = Constants.DoublePrecision)
    {
    ...
    {
        foreach (var attribute in Attributes().Where(attribute => !attribute.HasDefaultValue).OrderBy(attr => attr.Name.Length + attr.ToXmlString().Length).ThenBy(attr => attr.Name))
            writer.WriteAttributeString(attribute.Name, attribute.ToXmlString());

        if (this is IHasElementContent hasElementContent)
        {
            writer.WriteString(hasElementContent.Content);
        }
