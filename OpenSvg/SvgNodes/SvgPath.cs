...
public sealed partial class SvgPath : SvgVisual, IDisposable
{
        ...
        while ((verb = iterator.Next(points)) != SKPathVerb.Done)
            switch (verb)
            {
                ...
                case SKPathVerb.Cubic:
                    currentPoints.AddRange(ApproximateCubicBezier(points[0], points[1], points[2], points[3],
                        segments));
                    break;

                case SKPathVerb.Conic
                    : //Conic Bezier curves are not currently supported in SVGs, so this option will not be matched, but it is added for future compatibility
                    currentPoints.AddRange(ApproximateConicBezier(points[0], points[1], points[2],
                        iterator.ConicWeight(), segments));
                    break;
    ...
    /// <param name="segments">The number of segments to divide the curve into.</param>
    /// <returns>A sequence of points approximating the curve.</returns>
    private static IEnumerable<Point> ApproximateCubicBezier(SKPoint start, SKPoint control1, SKPoint control2,
        SKPoint end, int segments)
    {
        var points = new List<Point>();
    ...
    /// <param name="segments">The number of segments to divide the curve into.</param>
    /// <returns>A sequence of points approximating the curve.</returns>
    private static IEnumerable<Point> ApproximateConicBezier(SKPoint start, SKPoint control, SKPoint end, float weight,
        int segments)
    {
        var points = new List<Point>();
