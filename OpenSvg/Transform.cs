...
public readonly struct Transform : IEquatable<Transform>
{
    ...
    private Transform(Matrix3x2 matrix) => Matrix = matrix;
    ...
    /// </summary>
    public (double skewXDegrees, double skewYDegrees) Skew => (Math.Atan2(Matrix.M21, Matrix.M11), Math.Atan2(Matrix.M12, Matrix.M22));
    ...
    public static Transform CreateTranslation(double dx = 0, double dy = 0)
        => new(Matrix3x2.CreateTranslation((float)dx, (float)dy));
    ...
    public static Transform CreateScale(double scaleX = 1, double scaleY = 1)
        => new(Matrix3x2.CreateScale((float)scaleX, (float)scaleY));
    ...
    public static Transform CreateRotation(double angleDegrees = 0, double pivotX = 0, double pivotY = 0)
        => new(Matrix3x2.CreateRotation(ToRad(angleDegrees), new Vector2((float)pivotX, (float)pivotY)));
    ...
    public static Transform CreateSkew(double skewXDegrees = 0, double skewYDegrees = 0)
        => new(new Matrix3x2(1, ToRad(skewYDegrees), ToRad(skewXDegrees), 1, 0, 0));
    ...
    public static Transform CreateMatrix(double m11 = 1, double m12 = 0, double m21 = 0, double m22 = 1, double dx = 0, double dy = 0)
        => new(new Matrix3x2((float)m11, (float)m12, (float)m21, (float)m22, (float)dx, (float)dy));
    ...
    public Transform ComposeWith(Transform other)
        => new(Matrix3x2.Multiply(Matrix, other.Matrix));
