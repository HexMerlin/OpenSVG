...
/// </summary>
/// When a rectangle is drawn with rounded corners as a polygon
/// <see cref="SvgPolygon" />
/// , the number of points used to approximate a corner must be given (NumberOfCornerPoints)
/// NumberOfCornerPoints is ignored when the rectangle is drawn as a 'rect' using
/// <see cref="SvgRectangleAsRect" />
/// <remarks>
...
/// <param name="NumberOfCornerPoints">
///     Number of points used to approximate curved corners, when the rectangle is drawn as
///     a polygon
/// </param>
    ...
    public Polygon ToPolygon()
    {
        ...
        if (CornerRadius > Size.Width / 2 || CornerRadius > Size.Height / 2)
            throw new ArgumentException(
                "CornerRadius cannot be larger than half of the width or height of the rectangle");
        ...
        if (roundedCorners)
        {
            ...
            points.AddRange(CircularArc.CreateArcPoints(new Point(width - cr, cr), cr, 90, arcLength,
                NumberOfCornerPoints));

            points.Add(new Point(width, cr));
            points.Add(new Point(width, height - cr));

            points.AddRange(CircularArc.CreateArcPoints(new Point(width - cr, height - cr), cr, 0, arcLength,
                NumberOfCornerPoints));

            points.Add(new Point(width - cr, height));
            points.Add(new Point(cr, height));

            points.AddRange(CircularArc.CreateArcPoints(new Point(cr, height - cr), cr, 270, arcLength,
                NumberOfCornerPoints));
