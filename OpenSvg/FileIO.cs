...
public static class FileIO
{
    ...
    /// <returns>A tuple containing a boolean indicating whether the files are equal and a string with an error message if they are not.</returns>
    public static (bool IsEqual, string ErrorMessage) BinaryFileCompare(string filePath1, string filePath2)
    {
        ...
        if (fs1.Length != fs2.Length) return (false, $"Size mismatch: {filePath1} ({fs1.Length}), {filePath2} ({fs2.Length})");
        ...
        while ((bytesRead = fs1.Read(buffer1, 0, bufferSize)) > 0)
        {
            ...
            {
                if (buffer1[i] != buffer2[i]) return (false, $"Byte mismatch at {fs1.Position - bytesRead + i}");
            }
        }
